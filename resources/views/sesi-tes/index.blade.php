...
@section('content')
    ...
    <div style="background: var(--glass-bg); backdrop-filter: blur(16px); border: 1px solid var(--glass-border); border-radius: 16px; padding: 16px 20px; margin-bottom: 20px; display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
        <form method="GET" action="{{ route('sesi-tes.index') }}" id="filterForm" style="display: flex; align-items: center; gap: 12px; margin: 0;">
    ...
    <div style="display: flex; align-items: center; justify-content: flex-end; gap: 8px;">
        @if ($s->status === 'aktif')
            <form method="POST" action="{{ route('sesi-tes.update-status', $s->id) }}" style="margin: 0;">
                ...
                <button type="submit" style="color: var(--text-muted); background: rgba(255, 255, 255, 0.06); border: 1px solid var(--glass-border); padding: 8px 14px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer;">Selesaikan</button>
            </form>
        @endif
    ...
    <div id="sesiModal" style="display: none; position: fixed; inset: 0; z-index: 999; background: rgba(0,0,0,0.7); backdrop-filter: blur(6px); align-items: center; justify-content: center;">
        <div style="background: #0d1322; border: 1px solid var(--glass-border); border-radius: 20px; width: 100%; max-width: 480px; padding: 28px;">
            ...
            <form method="POST" action="{{ route('sesi-tes.store') }}">
                ...
                <div style="margin-bottom: 18px;">
                    ...
                    <select name="jenis_olahraga_id" required style="width: 100%; background: #161f33; border: 1px solid var(--glass-border); border-radius: 10px; padding: 12px 16px; color: var(--text-main); font-size: 14px; outline: none; box-sizing: border-box;">
